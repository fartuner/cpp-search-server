    ...
    vector<Document> FindAllDocuments(const set<string>& plus_words,
        ...
        for (const string &word : plus_words) {
            if (index.count(word) > 0) {
                // IDF
                double idf = log(static_cast<double>(document_count_) / index.at(word).size());

                for (const auto &[doc_id, tf_value] : index.at(word)) {
                    // TF-IDF
                    doc_relevance[doc_id] += tf_value * idf;
                }
            }
        }
        ...
        // Преобразуем карту в вектор документов
        vector<Document> matched_documents;
        for (const auto &[doc_id, relevance] : doc_relevance) {
            matched_documents.emplace_back(Document{doc_id, relevance});
        }
